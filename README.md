...
| `image` | 画像スライドショー（BGMを止めずに切り替え） | 不要 |
...
```
[file: media/image.jpg]           表示する画像ファイル
[duration: 10]                    表示秒数（デフォルト: 5）
[transition: 1.0]                 クロスフェードの秒数（デフォルト: 1.0）
[keepOverlay: true]               次のブロックへ移行時に画像を消さずに維持する
```
※BGMを途切らせずに画像を次々と切り替える演出に最適です。
...
- **`image` セグメント追加**: BGMを途切れさせずに画像を切り替える専用ブロックを新設
  - `duration`（表示秒数）, `transition`（クロスフェード秒数）, `keepOverlay` に対応
  - 作業用BGM動画のような演出が可能に
- **録画時の描画バグ修正**: オーバーレイ画像がWebM録画ファイルに出力されない問題を解決
- **実行制御バグ修正**: キャッシュ取り出し時の変数名誤りによるAIの沈黙バグを修正
- **台本パース修正**: Markdownヘッダーによるセグメント分割評価の不具合を修正
