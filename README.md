...
| **Style-Bert-VITS2** | 多彩な声質・自作モデル対応（:5000） | ✅ |
| **カスタム** | 任意のTTSサーバー | — |
| **ブラウザ内蔵** | Web Speech API（インストール不要） | ✗ |

> **VOICEVOX** は内蔵スピーカー（ずんだもん・四国めたん等）のみ使用可能で、オリジナル音声データには対応していません。  
> オリジナルの音声モデルを使いたい場合は **Style-Bert-VITS2** を選択してください。

### SBV2 の設定パラメータ

SBV2 を選択すると、以下の追加設定が表示されます：

| パラメータ | 説明 | デフォルト |
|-----------|------|:---:|
| **モデルID** (`model_id`) | 使用するモデルの番号 | 0 |
| **スタイル番号** (`speaker_id`) | モデル内のスタイル/話者番号 | 0 |

> ⚠️ SBV2 使用時、キャラクタースロットの「声」プルダウン（VOICEVOX用 speakerId）は**無視**されます。  
> 音声の切り替えは TTS設定のモデルID・スタイル番号で行ってください。
...
**ジェスチャーポーズ・ルビ記法・SBV2修正**

- **ジェスチャーポーズ**: .vroidpose からポーズデータを導入
  - 挙手・腕組み・バイバイ・ガッツポーズ（拳握り対応）
  - 感情タグ連動（5感情×3バリエーション、控えめモード対応）
  - Three.js基準の回転軸修正で自然な描写
- **ルビ記法**: `{表記|よみ}` で字幕はruby表示、音声合成は「よみ」を送信
- **auto-scenery セグメント**: ランダム風景写真で旅番組の演出強化
- **ランダム背景**: Picsum Photos（CC-0）との連携
- **SBV2 speakerId修正**: SBV2使用時にVOICEVOXのspeakerIdが誤送信される問題を修正
  - TTS設定の `model_id` / `speaker_id` を優先使用するよう変更
  - popup に SBV2 スタイル番号（speaker_id）設定フィールドを追加
- **AI曜日ハルシネーション対策**: プロンプトに正確な日付・曜日を注入
- **バグ修正**: ジェスチャー回転値蓄積バグ、TDZエラー、クレジット描画ガード等
...
- **エンドレストーク**: フリートーク連鎖機能
- **フリートーク話題拡充**: 内蔵30話+外部GitHub JSON 199話=229話、日付フィルタ対応
- **TTS外部辞書**: `tts-dictionary.json` の動的読み込み
- **WebMシーク対応**: Durationメタデータ修正
- **クレジット強化**: 使用全speakerId収集方式に変更、録画内クレジット表示
- **コメント応答キャラ振り分け**: キャラ名含むコメント→そのキャラの声で応答
- **感情リアクション**: 頷き・驚き仰け反り・happy揺れ・sad俯き・angry前のめり
- **キャラスロットUI統合**: VRMボタン削除・直接表示・声プルダウン選択
