...
OBS なしで番組を動画として録画できます。Chrome のタブキャプチャ機能を使い、画面に表示されているものをそのまま録画します。
...
6. 手動で停止したい場合は右上にマウスを動かすとボタンが再表示される

### 録画 Tips

- **開始ディレイを活用**: ポップアップで「⏱️ 5秒後に開始」を設定すれば、録画開始後に番組を再生してもカウントダウン中はクリーンな待機画面が録れる
- **ウィンドウサイズ**: ブラウザウィンドウのサイズがそのまま録画解像度になる。フルスクリーン（F11）推奨
- **再生中の録画**: 番組再生中でも録画ボタンを押せる（即座にキャプチャ開始）

### 注意

- 初回はタブ共有の許可ダイアログが出ます（毎回出ます）
- 「タブの音声も共有」にチェックを入れないでください（音声は内部でキャプチャ済み、二重になります）
- 音声は TTS・BGM・ジングルすべて含みます
